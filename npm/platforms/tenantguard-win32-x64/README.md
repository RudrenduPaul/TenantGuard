# tenantguard-win32-x64

This package contains the prebuilt TenantGuard binary for Windows on x64.
...
Install the main `tenantguard-cli` npm package instead:

```
npm install -g tenantguard-cli
```

npm resolves this package automatically as an optional dependency when you are on Windows x64, so the right binary lands on your `PATH` without any extra steps.
