...
var ruleOrder = []string{"TA01", "TA02", "TA03", "TA04", "TA05", "TA08", "TA09", "TA10", "TA12", "TA13", "TA14", "TA06", "TA07", "TA11", "TA15", "TA16"}
...
func (e *Evaluator) Evaluate(ctx context.Context, cfg *collector.CollectedConfig) ([]Finding, error) {
	...
	for _, r := range e.rules {
		...
		case "TA10":
			findings = append(findings, buildFindings(r.id, len(cfg.Agents), violationSet, func(i int) collector.Location {
				return cfg.Agents[i].Location
			})...)
		case "TA12":
